...
namespace Reboot_Framework
{
 ...
 bool unicode, bool casesensitive, bool percentage, bool compareToSavedScan, [MarshalAs(UnmanagedType.BStr)] string savedscanname);
        ...
        private IntPtr LoadInternalLibrary(string library)
        {
            string dirName = Path.Combine(Path.GetTempPath(), "RebootFramework" + Assembly.GetExecutingAssembly().GetName().Version.ToString());
            if (!Directory.Exists(dirName))
                Directory.CreateDirectory(dirName);

            string dllPath = Path.Combine(dirName, library);
            if (File.Exists(dllPath))
            {
                return LoadLibrary(dllPath);
            }

            using (Stream stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("Reboot_Framework.reboot-framework.dll"))
            {
                try
                {
        ...
        {
            libInst = LoadInternalLibrary("reboot-framework.dll");
